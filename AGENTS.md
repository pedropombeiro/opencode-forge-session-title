...
- `mise run lint_fix` — lint and auto-fix
...
2. The `Release` workflow runs release-please, which creates/updates a release PR (titled `chore(main): release opencode-forge-session-title <version>`). The PR bumps `package.json`, updates the manifest, and generates a changelog.
